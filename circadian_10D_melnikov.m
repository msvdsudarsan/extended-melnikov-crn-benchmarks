...
%% Model 5: Mammalian Circadian Rhythm (10D), epsilon = 0.0002
%% Uses Melnikov-Lyapunov surrogate for stability at extreme stiffness
...
params.vs=1.6;  params.vm=0.8;   params.Km=0.5;  params.ks=0.6;
params.vd=0.95; params.Kd=0.3;   params.k1=0.4;  params.k2=0.2;
params.KI=1.0;  params.n=4;      params.v1=0.7;  params.K1=0.5;
params.k3=0.4;  params.k4=0.2;
params.vs_p=1.5;  params.vm_p=0.75; params.Km_p=0.5;  params.ks_p=0.65;
params.vd_p=1.0;  params.Kd_p=0.35; params.k1_p=0.45; params.k2_p=0.22;
params.v1_p=0.75; params.K1_p=0.55; params.k3_p=0.42; params.k4_p=0.21;
params.epsilon=0.0002; params.delta=0.002;
...
for i = 1:length(mu_range)
    ...
    try
        [t,x]     = ode15s(@(t,x)circadian_rhs(t,x,params), tspan, x0, opts);
        M_simp(i) = melnikov_integral_circadian(t, x, lambda);
        M_lyap(i) = lyapunov_surrogate(t, x, lambda, x_eq);
    catch
...
else, mu_c = interp1(M_use(idx:idx+1), mu_range(idx:idx+1), 0); end
...
yline(0,'k--'); xlabel('\mu'); ylabel('Lyapunov surrogate');
title('Circadian 10D – Melnikov-Lyapunov Surrogate'); grid on;
save('circadian_10D_results.mat','results');
end

function dxdt = circadian_rhs(~, x, p)
dxdt     = zeros(10,1);
...
function M = melnikov_integral_circadian(t, x, lambda)
p_vals = x(:,1);
dx = diff(x(:,1)); dx = [dx; dx(end)];
w  = exp(-lambda*(t - t(1)));
M  = trapz(t, w.*p_vals.*abs(dx));
end

function L = lyapunov_surrogate(t, x, lambda, x_eq)
